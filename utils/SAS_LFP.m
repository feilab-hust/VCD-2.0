...
shift_4D=zeros( size(LFP_list,1),size(LFP_list,2),shift_times,shift_times );
for view_idx=1:shift_times*shift_times      
    shift_4D(:,:,index2(view_idx),index1(view_idx))= LFP_list(:,:,view_idx);
end
% Realignment
shfit_viewStack=zeros(Nnum,Nnum,base_h,base_w,shift_times,shift_times);
for i=1:Nnum
    for j=1:Nnum
        for a=1:base_h
            for b=1:base_w
...
shfit_viewMap=zeros(  base_h*shift_times,base_w*shift_times,Nnum,Nnum); 
for a=1:base_h
...
ViewStack=zeros( base_h*shift_times,base_w*shift_times,Nnum*Nnum  );
for ii=1:Nnum
    for jj=1:Nnum
