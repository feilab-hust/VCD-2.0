...
class Dataset:
    ...
    def _load_dataset(self, shuffle=True):
        ...
        #             type_name, img_file, str(img.shape), np.min(img), np.max(img)), end='')
        #         imgs.append(img)
        #     return np.asarray(imgs,dtype=np.float32), img_list[0:list_len]
        ...
        if self.is_hdf5:
            ...
            # self.training_Target3D, self.training_Target3D_list = _load_imgs_1(self.Target3D_path, fn=get_and_rearrange3d,
